    ...
    <div class="row">
        <div class="col-12">
            <p><a href="{{url('/')}}" class="text-decoration-none text-dark fw-bold">Property Market Live</a>
                >
                <a class="text-decoration-none text-dark fw-bold">Youtube videos</a>
            </p>
        </div>
    </div>
</div>
        ...
        <div class="col-8 col-xs-12 col-tab-12 article-main-area">
            ...
            @else
                @foreach($all_videos as $all_video)
                    <div class="video-article-main-area" style="margin-bottom:80px" id="down{{$all_video->id}}">
                    
                        <input type="hidden" value="{{ preg_match("#(?<=v=)[a-zA-Z0-9-]+(?=&)|(?<=v\/)[^&\n]+|(?<=v=)[^&\n]+|(?<=youtu.be/)[^&\n]+#", $all_video->youtube, $default_match) }}" />
                        ...
                        <div class="col-12 option-bar">
                            <div class="left-side">
                                ...
                                <span>{{ $all_video->created_at->format('M d Y') }}</span>
                            </div>
        ...
        <div class="col-4 col-xs-12 col-tab-12">
            ...
            @if(count($random_video) != 0)
                <div class="most-viewed-wrapper">
                    <h2 class="side-title">Trending Topics</h2>
                    <ul class="most-viewed">                        
                        @foreach($random_video as $random_vid)
                            <a href="#down{{$random_vid->id}}" style="text-decoration:none; color:#212529">
                                <li>
                                    <i class="fas fa-play"></i>
                                    <span class="most-viewed-txt">{{$random_vid->title}}</span>
                                </li>
                            </a>
                        @endforeach
                    </ul>
            ...
            <div class="trending-wrapper">
                <h2 class="side-title">Featured Topics</h2>

                @if(count($featured_video) == 0)                
                    @include('frontend.includes.not_found',[
                        'not_found_title' => 'Videos Not Found',
                        'not_found_description' => null,
                        'not_found_button_caption' => null
                ...
                @else
                    @foreach($featured_video as $featured_vid)
                        <a href="#down{{$featured_vid->id}}" style="text-decoration:none;color:black;">
                            <div class="trending-card">
                                <img src="{{ uploaded_asset($featured_vid->feature_image) }}" alt="">
                                <div class="trending-card-txt-wrapper">
                                    <div class="txt-inner-wrapper">
                                        <h3 style="overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">{{$featured_vid->title}}</h3>
                                        <p class="trending-card-date-time">{{ $featured_vid->created_at->format('M d Y') }}</p>
                                        <p style="overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical;">{{$featured_vid->description}}</p>
                                    </div>
